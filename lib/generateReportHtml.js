...
function moduloPublicidad(d) {
  ...
  const extra = hasTechData
    ...
    : '';
  return { titulo: 'Publicidad', score: d.scores?.publicidad, extra: extra + aclaracion, items, next, porque };
}
...
.radar-item, .opp-card, .detail-card, .hero, .comp-box, .op-headline-box, .op-ejemplo-box, .op-fuentes-box, .comp-insight-box, .canal-card, .dir-grupo, .unif-box, .ia-card, .chat-sim, .resenas-box {
  page-break-inside: avoid;
  break-inside: avoid;
}
/* Las tarjetas de módulo son altas: si se les prohíbe partirse, desperdician media
   página cada vez. Se permite que fluyan, pero manteniendo el encabezado junto a
   su contenido. */
.module-card { page-break-inside: auto; break-inside: auto; }
.module-head, .module-title-row { page-break-after: avoid; break-after: avoid; }
