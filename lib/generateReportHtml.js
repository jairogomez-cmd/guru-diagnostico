...
function creativoCardHtml(item) {
  ...
  if (item.snapshot) { // Meta
    ...
    if (!imagen && s.videos && s.videos[0]) { imagen = s.videos[0].video_preview_image_url || ''; formato = 'video'; }
  }
  if (item.cover_image) { imagen = item.cover_image; formato = 'video'; } // TikTok
  if (item.title && !texto) texto = item.title;

  const mediaHtml = imagen
    ? `<img class="ad-creativo-img" src="${esc(imagen)}" />`
    : `<div class="ad-creativo-placeholder">${formato === 'video' ? '🎬 Video' : '🖼️ Imagen'}</div>`;

  const fechasHtml = fechaDesde
    ? `<div class="ad-creativo-fechas">Desde ${esc(formatearFecha(fechaDesde))}${diasActivo ? ` · ${diasActivo} días activo` : ''}</div>`
    : '';

  return `<div class="ad-creativo-card">
    ${mediaHtml}
    ${texto ? `<p class="ad-creativo-texto">${esc(texto.slice(0, 140))}${texto.length > 140 ? '…' : ''}</p>` : ''}
    ${fechasHtml}
  </div>`;
}

function plataformaAdsCardHtml(nombre, data, colorHex) {
  const activo = data?.tieneAdsActivos;
  const anuncios = (data?.anuncios || []).slice(0, 3);
  return `<div class="ad-plataforma-card">
    <div class="ad-plataforma-head">
      <span class="ad-plataforma-nombre">${esc(nombre)}</span>
...
function generarRecomendacionesPublicidad(ads) {
  ...
  if (!g && !m && !t) {
    ...
    if (g && m) recos.push('Ya estás invirtiendo en los dos canales más importantes (Google y Meta) — el siguiente paso es optimizar, no sumar más plataformas: revisá qué anuncios traen mejores resultados y concentrá el presupuesto ahí.');
    else if (g && !m) recos.push('Tenés Google Ads activo pero no Meta — sumar Meta te permite hacer remarketing a quien ya visitó tu web, con una inversión inicial baja.');
    else if (m && !g) recos.push('Tenés Meta Ads activo pero no Google — Google captura a alguien que ya está buscando activamente lo que vendés, es un complemento natural a lo que ya hacés en redes.');
  ...
  // Insight de gobernanza: si el anunciante real de Google no coincide con
  // el nombre del negocio, vale la pena que el dueño lo sepa.
  const primerAdGoogle = (ads?.google?.anuncios || [])[0];
  const nombreAnunciante = primerAdGoogle?.advertiser?.name;
  if (nombreAnunciante) {
    recos.push(`Tu campaña de Google Ads está registrada bajo el nombre "${nombreAnunciante}" (probablemente tu agencia o quien te gestiona la pauta) — vale la pena confirmar que tenés acceso propio a esa cuenta publicitaria, no solo a través de un tercero.`);
  }
...
function publicidadRealHtml(d) {
  ...
  return `<div class="module-card page-start">
    ...
    ${plataformaAdsCardHtml('Google Ads', ads.google, '#4285F4')}
    ${plataformaAdsCardHtml('Meta Ads (Facebook + Instagram)', ads.meta, '#1877F2')}
    ${plataformaAdsCardHtml('TikTok Ads', ads.tiktok, '#000000')}

    <div class="ad-recos-box">
      <div class="ad-recos-title">Cómo mejorar tu pauta publicitaria</div>
...
.ad-plataforma-card { background: var(--guru-bg); border-radius: var(--radius-md); padding: 14px 16px; margin-bottom: 12px; }
...
.ad-plataforma-badge { font: 700 10px var(--font-display); letter-spacing: .04em; padding: 3px 10px; border-radius: 999px; }
...
.ad-plataforma-fuente { font-size: 9.5px; color: var(--guru-text-muted); margin-top: 8px; }
.ad-creativos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
.ad-creativo-card { background: var(--guru-surface); border: 1px solid var(--guru-border); border-radius: var(--radius-sm); overflow: hidden; }
.ad-creativo-img { width: 100%; height: 90px; object-fit: cover; display: block; background: #eee; }
.ad-creativo-placeholder { width: 100%; height: 90px; display: flex; align-items: center; justify-content: center; background: #EEF0F5; color: var(--guru-text-muted); font-size: 11px; }
.ad-creativo-texto { font-size: 10px; color: var(--guru-text-secondary); padding: 6px 8px 2px; line-height: 1.4; margin: 0; }
.ad-creativo-fechas { font-size: 9px; color: var(--guru-text-muted); padding: 2px 8px 8px; }
