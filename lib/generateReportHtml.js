...
// ── Velocímetro del hero: reemplaza el anillo plano por un dial con zonas
// de color y aguja, en la paleta de marca (no en los colores del ejemplo de
// referencia). Usa la misma técnica de arco + pathLength=100 que ya se usa
// en gaugeSemiSvg, así que no suma dependencias nuevas. ──
function heroGaugeSvg(score, hex) {
  const clamped = Math.max(0, Math.min(100, score));
  const t = clamped / 100;
  const cx = 100, cy = 92, R = 78, needleLen = 58;
  const nx = (cx - needleLen * Math.cos(t * Math.PI)).toFixed(1);
  const ny = (cy - needleLen * Math.sin(t * Math.PI)).toFixed(1);
  return `<svg viewBox="0 0 200 128" width="192" height="123">
    <defs>
      <linearGradient id="heroGaugeZones" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0%" stop-color="#E94560"/>
        <stop offset="38%" stop-color="#E08A3C"/>
        <stop offset="62%" stop-color="#F5A623"/>
        <stop offset="100%" stop-color="#0F9B8E"/>
      </linearGradient>
    </defs>
    <path d="M18,${cy} A${R},${R} 0 0 1 182,${cy}" fill="none" stroke="url(#heroGaugeZones)" stroke-width="7" stroke-linecap="round" opacity=".38"/>
    <path d="M18,${cy} A${R},${R} 0 0 1 182,${cy}" fill="none" stroke="${hex}" stroke-width="7" stroke-linecap="round" pathLength="100" stroke-dasharray="100" stroke-dashoffset="${100 - clamped}"/>
    <line x1="${cx}" y1="${cy}" x2="${nx}" y2="${ny}" stroke="white" stroke-width="3.4" stroke-linecap="round"/>
    <circle cx="${cx}" cy="${cy}" r="6.5" fill="white"/>
    <circle cx="${cx}" cy="${cy}" r="2.6" fill="#1A1A2E"/>
    <text x="${cx}" y="${cy - 30}" text-anchor="middle" font-family="Plus Jakarta Sans, sans-serif" font-size="34" font-weight="800" fill="white">${clamped}</text>
    <text x="${cx}" y="${cy - 12}" text-anchor="middle" font-family="DM Sans, sans-serif" font-size="10.5" letter-spacing="1" fill="rgba(255,255,255,.65)">DE 100</text>
  </svg>`;
}
function iconQuote() {
...
.gauge-tier { font: 700 13px var(--font-display); color: white; text-align: center; max-width: 168px; margin-top: 4px; }
