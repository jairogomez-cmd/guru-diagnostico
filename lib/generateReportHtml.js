...
function iconArrowUpRight() { return `<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;flex-shrink:0;"><path d="M7 17L17 7M7 7h10v10"/></svg>`; }
...
function moduloPublicidad(d) {
  ...
  const items = [
    { tipo: p.googleAdsDetectado ? 'ok' : 'warn', texto: p.googleAdsDetectado ? `Etiqueta de Google Ads detectada en tu sitio${p.googleAdsEsReal ? ' — confirmado en vivo en Google Ads Transparency Center' : ''}` : 'No detectamos la etiqueta de Google Ads en el sitio analizado' },
    ...
    { tipo: p.metaAdsDetectado ? 'ok' : 'warn', texto: p.metaAdsDetectado ? `Pixel de Meta detectado — Facebook e Instagram pueden medir tus resultados${p.metaAdsEsReal ? ' — confirmado en vivo en Meta Ad Library' : ''}` : 'No detectamos el Pixel de Meta en el sitio analizado' },
  ];
  ...
  // Aclaración honesta: si hay Tag Manager pero ninguna señal directa, el dato
  // puede estar configurado ADENTRO del contenedor de GTM (no visible en el
  // HTML que descargamos) -- distinto de "no tiene publicidad activa".
  if (pt.posibleTagOcultoEnGTM || pt.posiblePixelOcultoEnGTM) {
    const cuales = [pt.posibleTagOcultoEnGTM && 'Google Ads', pt.posiblePixelOcultoEnGTM && 'el Pixel de Meta'].filter(Boolean).join(' y ');
    items.push({ tipo: 'warn', texto: `Tenés Google Tag Manager activo, así que es posible que ${esc(cuales)} esté${cuales.includes(' y ') ? 'n' : ''} configurado ahí adentro sin que se vea desde acá — confirmalo directo en tu cuenta de Tag Manager o preguntale a tu asesor.` });
  }
  ...
  const tieneVerificacionReal = !!(p.googleAdsEsReal || p.metaAdsEsReal);
  const aclaracion = tieneVerificacionReal
    ? `<p class="modulo-alcance">Qué miramos acá: además de revisar las etiquetas de medición instaladas en <strong>${esc((d.website || '').replace(/^https?:\/\//, ''))}</strong>, confirmamos en vivo directo contra Google Ads Transparency Center y Meta Ad Library — así detectamos campañas activas aunque corran desde otro dominio o una landing separada. Más abajo en este informe tenés el detalle completo, con las creatividades reales.</p>`
    : `<p class="modulo-alcance">Qué miramos acá: las etiquetas de medición instaladas en <strong>${esc((d.website || '').replace(/^https?:\/\//, ''))}</strong>. Si tus campañas corren desde otro dominio o subdominio (por ejemplo, una landing de marketing aparte), no las vamos a ver desde acá — comentáselo a tu asesor para revisarlo juntos.</p>`;
...
// ══ Salud del contenido (Tanda 1) ══════════════════════════════
function tierChip(score) {
  if (score >= 70) return { cls: 'ok', hex: 'var(--guru-teal)' };
  if (score >= 45) return { cls: 'warn', hex: 'var(--guru-gold)' };
  return { cls: 'bad', hex: 'var(--guru-accent)' };
}

function saludContenidoHtml(d) {
  const ha = d.htmlAnalysis || {};
  if (!ha || Object.keys(ha).length === 0) return '';

  const filas = [];

  // Títulos y meta descripciones
  if (ha.titlesDup || ha.descsDup) {
    const partes = [];
    if (ha.titlesDup) partes.push('el título de página');
    if (ha.descsDup) partes.push('la meta descripción');
    filas.push({ tipo: 'bad', html: `<strong>${esc(partes.join(' y '))} se repite</strong> en más de una página — Google penaliza levemente el contenido duplicado entre páginas de un mismo sitio.` });
  } else if (ha.titles && ha.titles.length) {
    filas.push({ tipo: 'ok', html: `<strong>Títulos y meta descripciones</strong> sin duplicados detectados.` });
  }

  // Encabezados
  if (ha.h1count === 0) {
    filas.push({ tipo: 'bad', html: `<strong>Sin encabezado H1</strong> en la portada — dificulta que Google entienda de qué trata la página principal.` });
  } else if (ha.h1count > 1) {
    filas.push({ tipo: 'warn', html: `<strong>${ha.h1count} encabezados H1</strong> en la misma página — lo ideal es uno solo por página.` });
  } else {
    filas.push({ tipo: 'ok', html: `<strong>Estructura de encabezados</strong> correcta (un H1 por página).` });
  }
  if (ha.hVacios > 0) {
    filas.push({ tipo: 'warn', html: `<strong>${ha.hVacios} encabezado${ha.hVacios > 1 ? 's' : ''} vacío${ha.hVacios > 1 ? 's' : ''}</strong> — etiquetas de título sin texto adentro, confunden a Google y a los lectores de pantalla.` });
  }

  // Imágenes sin alt
  if (ha.imgTotal > 0) {
    if (ha.imgSinAlt > 0) {
      filas.push({ tipo: 'warn', html: `<strong>${ha.imgSinAlt} de ${ha.imgTotal} imágenes</strong> sin texto alternativo (alt) — afecta accesibilidad y le resta contexto a Google e IA sobre qué muestra cada imagen.` });
    } else {
      filas.push({ tipo: 'ok', html: `<strong>Todas las imágenes</strong> tienen texto alternativo configurado.` });
    }
  }

  // Enlaces
  if (ha.enlacesTextoAmbiguo > 0) {
    filas.push({ tipo: 'warn', html: `<strong>${ha.enlacesTextoAmbiguo} texto${ha.enlacesTextoAmbiguo > 1 ? 's' : ''} de enlace</strong> se repite apuntando a destinos distintos — confunde a quien navega y a los buscadores sobre a dónde lleva cada uno.` });
  }
  if (ha.enlacesSinTexto > 0) {
    filas.push({ tipo: 'warn', html: `<strong>${ha.enlacesSinTexto} enlace${ha.enlacesSinTexto > 1 ? 's' : ''}</strong> sin texto accesible (ni texto visible ni descripción en la imagen) — invisible para lectores de pantalla.` });
  }

  // Contacto
  if (ha.ctaOk) {
    const canales = [ha.tieneTel && 'teléfono', ha.tieneEmail && 'email', ha.tieneWA && 'WhatsApp'].filter(Boolean);
    filas.push({ tipo: 'ok', html: `<strong>Datos de contacto con clic directo</strong> (${esc(canales.join(', '))}) — facilita que un visitante te contacte sin fricción.` });
  } else {
    filas.push({ tipo: 'bad', html: `<strong>Sin botones de contacto directo</strong> (clic para llamar, escribir o mandar WhatsApp) — cada paso extra que le pedís al visitante es una oportunidad de que se vaya sin contactarte.` });
  }

  if (!filas.length) return '';

  const filasHtml = filas.map(f => {
    const icon = f.tipo === 'ok' ? dirIconOk() : f.tipo === 'bad' ? dirIconNo() : dirIconWarn();
    return `<div class="cq-row"><span class="cq-icon">${icon}</span><div class="cq-row-text">${f.html}</div></div>`;
  }).join('');

  // Tarjetas de arriba: legibilidad + contenido (con salvedad si el sitio es JS-heavy)
  const statCards = [];
  if (ha.contenidoMedible === false) {
    statCards.push(`<div class="cq-card"><div class="cq-card-label">Contenido y legibilidad</div><div class="cq-card-detail" style="margin-top:6px">Este sitio carga su contenido con JavaScript (React/Next/Angular/Vue) — no pudimos medirlo leyendo el HTML directamente, así que no mostramos estos números para no arriesgar un dato incorrecto.</div></div>`);
  } else {
    const legTier = tierChip(ha.legibilidad || 0);
    statCards.push(`<div class="cq-card"><div class="cq-card-label">Legibilidad</div><div class="cq-card-value" style="color:${legTier.hex}">${ha.legibilidad ?? '—'}<span style="font-size:13px;color:var(--guru-text-muted)">/100</span></div><div class="cq-card-detail">${(ha.legibilidad || 0) >= 70 ? 'Texto fácil de leer rápido, incluso desde el celular.' : (ha.legibilidad || 0) >= 45 ? 'Dificultad media — algunos párrafos son largos para leer rápido.' : 'Texto denso — párrafos y oraciones largas dificultan una lectura rápida.'}</div></div>`);
    statCards.push(`<div class="cq-card"><div class="cq-card-label">Contenido de la portada</div><div class="cq-card-value" style="color:var(--guru-purple)">${(ha.palabras || 0).toLocaleString('es-AR')}</div><div class="cq-card-detail">palabras visibles al cargar la página.</div></div>`);
  }

  return `<div class="module-card page-start">
    <div class="module-top">
      <span class="module-name-row">${moduloIconBadge('construccion', 'var(--guru-purple)', 28)}<span class="module-name">Salud del contenido</span></span>
    </div>
    <p style="font-size:12.5px;color:var(--guru-text-secondary);margin:2px 0 14px;">Revisamos cómo está armado el contenido de tu sitio — lo que ven tus visitantes, y lo que leen Google y las IA.</p>
    <div class="cq-grid">${statCards.join('')}</div>
    <div class="cq-list">${filasHtml}</div>
    <div class="cq-cross"><strong>Esto también le importa a las IA:</strong> además de estos puntos, revisamos si tu sitio tiene <strong>datos estructurados (Schema)</strong> — ${ha.schemaLB ? `<span style="color:var(--guru-teal);font-weight:700">detectado</span>` : `<span style="color:var(--guru-accent);font-weight:700">no detectado</span>`} — y si permite el acceso de los rastreadores de IA — ese detalle está en la sección de Presencia en Inteligencia Artificial de este informe.</div>
  </div>`;
}

// ══ Buenas prácticas de AISO (contenido educativo, fijo) ═══════
function buenasPracticasAisoHtml() {
  const puntos = [
    { t: 'Datos estructurados (Schema)', d: 'Un código invisible en el sitio que le confirma a Google y a las IA qué es el negocio, dónde está y qué ofrece — sin esto, tienen que "adivinar".' },
    { t: 'Contenido real y actualizado', d: 'Texto verdadero en la página (no solo imágenes o JavaScript vacío al cargar) — es lo primero que una IA puede leer directamente.' },
    { t: 'Preguntas frecuentes en formato reconocible', d: 'Una sección de FAQ bien marcada es de lo que más citan las IA al armar una respuesta.' },
    { t: 'Acceso permitido a los rastreadores de IA', d: 'Bots como GPTBot, ClaudeBot y PerplexityBot necesitan permiso explícito en el sitio — si están bloqueados, esa IA no puede leer el contenido.' },
    { t: 'Reputación consistente', d: 'Buena cantidad de reseñas, con la misma información (nombre, dirección, teléfono) en todos los directorios donde aparece el negocio.' },
  ];
  const filas = puntos.map((p, i) => `<div class="bp-item"><span class="bp-num">${i + 1}</span><div><strong>${esc(p.t)}</strong><p>${esc(p.d)}</p></div></div>`).join('');
  return `<div class="module-card">
    <div class="module-top">
      <span class="module-name-row">${iconSparkle()}<span class="module-name">Lo que toda empresa necesita para un buen AISO</span></span>
    </div>
    <p style="font-size:12.5px;color:var(--guru-text-secondary);margin:2px 0 14px;">Estos son los 5 pilares que determinan si una Inteligencia Artificial puede encontrar, entender y recomendar un negocio.</p>
    <div class="bp-list">${filas}</div>
    <div class="next-step"><strong>¿Querés profundizar en esto para tu negocio?</strong> Hablá con tu asesor Guru — puede armar un plan puntual para reforzar cada uno de estos puntos.</div>
  </div>`;
}

// ══ Publicidad Real — creatividades, fechas y recomendaciones ══════
...
function creativoCardHtml(item) {
  // Normaliza los 3 formatos distintos (Google / Meta / TikTok) a una
  // sola tarjeta visual: imagen si hay, si no un placeholder por formato.
  let imagen = '';
  let texto = '';
  let fechaDesde = '';
  let diasActivo = null;
  let formatoEtiqueta = '';
  let esScreenshot = false; // las de Google son capturas de pantalla del anuncio, no fotos -- van con "contain", no "cover"

  if (item.target_domain !== undefined) { // Google ad_creative
    fechaDesde = item.first_shown_datetime;
    diasActivo = item.total_days_shown;
    formatoEtiqueta = item.format === 'text' ? 'Búsqueda' : 'Display';
    if (item.image?.link) { imagen = item.image.link; esScreenshot = true; }
  }
  if (item.snapshot) { // Meta
    const s = item.snapshot;
    texto = (s.body && s.body.text) || s.link_description || '';
    if (s.images && s.images[0]) imagen = s.images[0].original_image_url || s.images[0].resized_image_url || '';
    if (!imagen && s.videos && s.videos[0]) { imagen = s.videos[0].video_preview_image_url || ''; formatoEtiqueta = 'Video'; }
    if (!formatoEtiqueta) formatoEtiqueta = Array.isArray(item.publisher_platforms) && item.publisher_platforms.length
      ? item.publisher_platforms.map(p => p === 'instagram' ? 'Instagram' : p === 'facebook' ? 'Facebook' : p).join(' + ')
      : 'Facebook + Instagram';
  }
  if (item.cover_image) { imagen = item.cover_image; formatoEtiqueta = 'Video'; } // TikTok
  if (item.title && !texto) texto = item.title;

  // Mensaje del placeholder según el motivo real de por qué no hay imagen
  // -- "anuncio de texto" no es lo mismo que "no pudimos traer la vista previa".
  const placeholderTexto = formatoEtiqueta === 'Búsqueda'
    ? '📝 Anuncio de texto (sin imagen)'
    : formatoEtiqueta === 'Video'
      ? '🎬 Video'
      : '🖼️ Vista previa no disponible';

  const mediaHtml = imagen
    ? `<img class="ad-creativo-img${esScreenshot ? ' contain' : ''}" src="${esc(imagen)}" onerror="this.closest('.ad-creativo-media').innerHTML='<div class=&quot;ad-creativo-placeholder&quot;>🖼️ Vista previa no disponible</div>'" />`
    : `<div class="ad-creativo-placeholder">${placeholderTexto}</div>`;

  return `<div class="ad-creativo-card">
    <div class="ad-creativo-media${esScreenshot ? ' screenshot' : ''}">${mediaHtml}</div>
    <div class="ad-creativo-body">
      ${formatoEtiqueta ? `<span class="ad-creativo-formato">${esc(formatoEtiqueta)}</span>` : ''}
      ${texto ? `<p class="ad-creativo-texto">${esc(texto)}</p>` : ''}
      ${fechaDesde ? `<div class="ad-creativo-fechas">Desde ${esc(formatearFecha(fechaDesde))}${diasActivo ? ` · ${diasActivo} días activo` : ''}</div>` : ''}
    </div>
  </div>`;
}

function plataformaAdsCardHtml(nombre, data) {
  const activo = data?.tieneAdsActivos;
  const anuncios = (data?.anuncios || []).slice(0, 6);
  const totalReal = data?.cantidadAnuncios ?? anuncios.length;
  return `<div class="ad-plataforma-card">
    <div class="ad-plataforma-head">
      <span class="ad-plataforma-nombre">${esc(nombre)}${activo && totalReal ? ` <span class="ad-plataforma-conteo">(${totalReal} activo${totalReal === 1 ? '' : 's'})</span>` : ''}</span>
      <span class="ad-plataforma-badge ${activo ? 'si' : 'no'}">${activo ? 'ACTIVO' : 'NO DETECTADO'}</span>
    </div>
    ${activo ? `<div class="ad-creativos-grid">${anuncios.map(creativoCardHtml).join('')}</div>` : `<p class="ad-plataforma-vacio">No encontramos anuncios activos verificables en este momento.</p>`}
    ${activo && totalReal > anuncios.length ? `<p class="ad-plataforma-mas">+ ${totalReal - anuncios.length} anuncio${totalReal - anuncios.length === 1 ? '' : 's'} más activos, no mostrados acá por espacio.</p>` : ''}
    <div class="ad-plataforma-fuente">${esc(data?.fuente || '')}</div>
  </div>`;
}

function generarRecomendacionesPublicidad(ads) {
  const g = ads?.google?.tieneAdsActivos;
  const m = ads?.meta?.tieneAdsActivos;
  const t = ads?.tiktok?.tieneAdsActivos;
  const recos = [];

  // ── Nivel 1: brechas reales (canales sin activar) ──
  if (!g && !m && !t) {
    recos.push('Hoy no encontramos publicidad activa en ninguna plataforma verificada — es el canal más rápido para conseguir clientes que ya están buscando lo que ofrecés en este momento, no en 3 meses.');
  } else {
    if (g && !m) recos.push('Tenés Google Ads activo pero no Meta — sumar Meta te permite hacer remarketing a quien ya visitó tu web, con una inversión inicial baja.');
    if (m && !g) recos.push('Tenés Meta Ads activo pero no Google — Google captura a alguien que ya está buscando activamente lo que vendés, es un complemento natural a lo que ya hacés en redes.');
    if (!t) recos.push('No detectamos presencia en TikTok Ads — si tu público incluye una audiencia más joven, puede ser un canal para probar con presupuesto chico antes de escalar.');
  }

  // ── Nivel 2: optimización -- estas aparecen SIEMPRE que el canal esté
  // activo, incluso con el puntaje en 100. Un canal activo nunca está
  // "terminado": siempre hay una siguiente mejora concreta que proponer.
  if (g) {
    const anunciosGoogle = ads.google.anuncios || [];
    const formatos = new Set(anunciosGoogle.map(a => a.format));
    const masAntiguo = anunciosGoogle.reduce((max, a) => Math.max(max, a.total_days_shown || 0), 0);
    if (formatos.size > 1) {
      recos.push('Ya combinás anuncios de Búsqueda y de Display en Google — para seguir escalando, revisá cuál de los dos formatos te trae mejores resultados y priorizá presupuesto ahí.');
    } else {
      recos.push(`Hoy solo usás formato de ${formatos.has('text') ? 'Búsqueda' : 'Display'} en Google — sumar el otro formato amplía dónde te ve un cliente potencial, sin competir por las mismas palabras clave.`);
    }
    if (masAntiguo >= 90) {
      recos.push(`Tu anuncio de Google más antiguo lleva ${masAntiguo} días activo sin cambios — vale la pena renovar el copy o la imagen: un mismo aviso por mucho tiempo pierde efectividad (fatiga de anuncio) aunque siga corriendo.`);
    }
  }
  if (m) {
    recos.push('Con Meta, revisá cada 2-3 semanas la frecuencia de tus anuncios (cuántas veces le aparece a la misma persona) — si sube mucho sin que compren, es momento de renovar la creatividad antes de seguir gastando presupuesto en la misma pieza.');
  }
  if (g && m) {
    recos.push('Con los dos canales principales activos, el siguiente paso es dejar de "sumar plataformas" y empezar a optimizar: probá 2-3 variantes de cada anuncio (A/B testing) y quedate solo con la que mejor convierte.');
  }

  // ── Insight de gobernanza: anunciante distinto a la marca ──
  const primerAdGoogle = (ads?.google?.anuncios || [])[0];
  const nombreAnunciante = primerAdGoogle?.advertiser?.name;
  if (nombreAnunciante) {
    recos.push(`Tu campaña de Google Ads está registrada bajo el nombre "${esc(nombreAnunciante)}" (probablemente tu agencia o quien te gestiona la pauta) — vale la pena confirmar que tenés acceso propio a esa cuenta publicitaria, no solo a través de un tercero.`);
  }

  return recos;
}

function publicidadRealHtml(d) {
  const ads = d.publicidad?.publicidadReal;
  if (!ads) return '';

  const fecha = ads.fechaConsulta
    ? new Date(ads.fechaConsulta).toLocaleString('es-AR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
    : '';

  const recos = generarRecomendacionesPublicidad(ads);

  return `<div class="module-card page-start">
    <div class="module-top">
      <span class="module-name-row">${iconSparkle()}<span class="module-name">Publicidad — verificado en vivo</span></span>
      ${fecha ? `<span class="ad-fecha-verificacion">Verificado el ${esc(fecha)} hs</span>` : ''}
    </div>
    <p style="font-size:12.5px;color:var(--guru-text-secondary);margin:2px 0 14px;">Consultamos directo a Google Ads Transparency Center, Meta Ad Library y TikTok Ads Library — esto es lo que encontramos, con las creatividades reales que están corriendo hoy.</p>

    ${plataformaAdsCardHtml('Google Ads', ads.google)}
    ${plataformaAdsCardHtml('Meta Ads (Facebook + Instagram)', ads.meta)}
    ${plataformaAdsCardHtml('TikTok Ads', ads.tiktok)}

    <div class="ad-recos-box">
      <div class="ad-recos-title">Cómo seguir mejorando tu pauta publicitaria</div>
      <ul class="reco-list">${recos.map((r, i) => `<li><span class="reco-num">${i + 1}</span> ${r}</li>`).join('')}</ul>
    </div>
  </div>`;
}
...
function plataformaIaCard(nombre, aparece, posicion, score, nota, fragmento, fuente, esEstimacion) {
  const hex = score >= 70 ? '#0F9B8E' : score >= 45 ? '#F5A623' : '#E94560';
  const r = 26, c = 2 * Math.PI * r, dash = (Math.max(0, Math.min(100, score)) / 100) * c;
  const verifBadge = esEstimacion === false
    ? `<span class="ia-verif ia-verif-real">${iconCheck('#0F9B8E')} Verificado en vivo</span>`
    : `<span class="ia-verif ia-verif-est">Estimado por señales del negocio</span>`;
  return `<div class="ia-card">
    ...
    ${aparece && posicion ? `<div class="ia-card-pos"><span class="ia-pos-num">#${posicion}</span><span class="ia-pos-lbl">posición${esEstimacion === false ? '' : ' estimada'}<br>en la lista de recomendados</span></div>` : `<div class="ia-card-pos ia-card-pos-off"><span class="ia-pos-lbl">Hoy no aparecerías entre los recomendados</span></div>`}
    ${fragmento ? `<p class="ia-card-quote">&ldquo;${esc(fragmento)}&rdquo;</p>` : ''}
    ${fuente ? `<a class="ia-card-fuente" href="${esc(fuente)}" target="_blank" rel="noopener">Ver fuente citada ${iconArrowUpRight()}</a>` : ''}
...
function conversacionSimuladaHtml(d) {
  const a = d.aiso || {};
  // Si ya tenemos una pregunta real verificada en vivo (aisoReal / N14-FINAL), mostramos esa version.
  if (a.pregunta) {
    const fecha = a.fechaConsulta
      ? new Date(a.fechaConsulta).toLocaleDateString('es-AR', { day: 'numeric', month: 'long', year: 'numeric' })
      : '';
    const fechaHora = a.fechaConsulta
      ? new Date(a.fechaConsulta).toLocaleString('es-AR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
      : '';
    return `
    <div class="chat-sim">
      <div class="chat-sim-head">
        <span class="chat-sim-tag chat-sim-tag-real">${iconCheck('#ffffff')} Verificado en vivo</span>
        <span class="chat-sim-title">Así respondieron las IA hoy</span>
        ${fechaHora ? `<span class="chat-sim-fecha">${esc(fechaHora)} hs</span>` : ''}
      </div>
      <div class="chat-bubble chat-user"><div class="chat-avatar chat-avatar-user">C</div><div class="chat-text">${esc(a.pregunta)}</div></div>
      <div class="chat-cols">
        ${['chatgpt','gemini','claude'].map(p => {
          const resumen = a[`${p}_resumen`];
          const top3 = Array.isArray(a[`${p}_top3`]) ? a[`${p}_top3`] : [];
          if (!resumen && !top3.length) return '';
          const label = { chatgpt: 'ChatGPT', gemini: 'Gemini', claude: 'Claude' }[p];
          return `<div class="chat-col">
            <div class="chat-col-head"><div class="chat-avatar chat-avatar-ai">${iconSparkle()}</div><span class="chat-col-label">${label}</span></div>
            ${resumen ? `<p class="chat-col-text">${esc(resumen)}</p>` : ''}
            ${top3.length ? `<div class="chat-col-top3"><span class="chat-col-top3-label">Recomienda:</span><ol>${top3.map(n => `<li>${esc(n)}</li>`).join('')}</ol></div>` : ''}
          </div>`;
        }).join('')}
      </div>
      <p class="chat-sim-foot">Le hicimos esta pregunta, en vivo, a ChatGPT, Gemini y Claude${fecha ? `, el ${fecha}` : ''} — la misma que haría un cliente real buscando tu tipo de negocio. Los resultados de cada una están arriba, con la respuesta textual y la fuente que citó cada modelo.</p>
    </div>`;
  }
  // Respaldo: comportamiento anterior (reconstrucción con IA) para diagnósticos viejos sin datos reales.
...
function aisoSectionHtml(d) {
  ...
  const notaClaude = 'Prioriza fuentes directas y actualizadas que encuentra buscando en la web.';
  const est = a.esEstimacionPorProveedor || {};
  ...
  <div class="aiso-section page-start">
    ...
    <div class="ia-cards">
      ${plataformaIaCard('ChatGPT', a.chatgpt_aparece, a.chatgpt_posicion, a.score_chatgpt || 0, notaChat, a.chatgpt_fragmento, (a.chatgpt_fuentes || [])[0], est.chatgpt)}
      ${plataformaIaCard('Gemini', a.gemini_aparece, a.gemini_posicion, a.score_gemini || 0, notaGem, a.gemini_fragmento, (a.gemini_fuentes || [])[0], est.gemini)}
      ${plataformaIaCard('Claude', a.claude_aparece, a.claude_posicion, a.score_claude || 0, notaClaude, a.claude_fragmento, (a.claude_fuentes || [])[0], est.claude)}
    </div>
    ...
    ${schemaHtml}
    ${botsHtml}
    ${(!a.chatgpt_aparece && !a.gemini_aparece && !a.claude_aparece) ? `<div class="aiso-urgencia">${iconWarn('#ffffff')} Ahora mismo, si un cliente le pregunta a la inteligencia artificial por tu rubro, <strong>le está recomendando a la competencia en tu lugar.</strong></div>` : ''}
...
function canalPrioritarioCard(dir) {
  ...
  } else if (dir.siteId === 'BING' && !dir.fuenteAlternativa) {
    // Bing tiene un retraso de sincronización documentado del lado de Yext
    // (a diferencia de otros directorios) -- cuando el dato viene del Scan
    // estimado (no de Listings real), evitamos un "no apareces" tajante
    // que puede ser falso.
    detalle = 'Bing suele tardar más en reflejar cambios recientes — confirmalo directo en bing.com/places antes de asumir que no estás.';
  } else {
...
// Por qué le sirve a este negocio aparecer en cada tipo de canal —
// argumento breve y concreto, pensado para que el vendedor lo use tal cual.
const BENEFICIOS_POR_TIPO = {
  'Buscador y mapas': 'Es donde más gente busca directo — cada minuto sin ficha ahí es un cliente que encuentra a la competencia primero.',
  'Buscador': 'Motor de búsqueda con millones de usuarios propios — no depende de Google para que te encuentren.',
  'Red social': 'Antes de comprar, la mayoría revisa tus redes para confirmar que sos un negocio real y activo.',
  'Mapas y asistentes': 'Cuando alguien pide indicaciones para llegar desde el celular, esta es la fuente que se usa.',
  'Mapas y navegación': 'Cuando alguien pide indicaciones para llegar, esta es la fuente que consulta su navegador o su auto.',
  'Datos de ubicación': 'Alimenta en silencio a decenas de apps y asistentes que muestran negocios cercanos — no se ve, pero define si aparecés en otros lados.',
  'Inteligencia artificial': 'Cada vez más gente le pregunta directo a una IA dónde comprar, en vez de buscar en Google.',
  'Asistente de voz': '"Alexa, encontrame un..." — si no estás acá, ese cliente ni se entera que existís.',
  'Asistente de IA': 'Un canal más por donde una inteligencia artificial puede recomendarte, sin que vos lo sepas.',
  'Directorio local': 'Cada directorio es un link más apuntando a tu negocio — construye la confianza que Google necesita para ubicarte mejor.',
  'Directorio comercial': 'Presencia en una red de confianza reconocida, que refuerza tu legitimidad frente a un cliente que investiga antes de comprar.',
  'Turismo y reseñas': 'Clave si tu negocio recibe visitantes: acá es donde deciden antes de elegir a dónde ir.',
  'Reservas gastronómicas': 'Es el canal donde el cliente ya decidió — solo falta que te encuentre y pueda reservar.',
  'Reservas de alojamiento': 'Es el canal donde el cliente ya decidió viajar — solo falta que te encuentre y pueda reservar.',
  'Reseñas': 'Reseñas que suman a tu reputación general en internet, sin que tengas que pedirlas vos.',
};

// Caja destacada: fichas que Yext confirma que alimentan a otra marca
// (ej. Google Business Profile -> Waze y Gemini). Solo aparece cuando
// hay al menos un dato real -- si no hay nada, no se muestra nada.
function integracionesDestacadasHtml(yx) {
  const items = Array.isArray(yx.integracionesDestacadas) ? yx.integracionesDestacadas : [];
  if (!items.length) return '';
  const filas = items.map(i =>
    `<div class="integ-fila"><strong>${esc(i.nombre)}</strong> también alimenta a ${i.marcas.map(esc).join(' y ')}</div>`
  ).join('');
  return `<div class="integ-box">
    <div class="integ-title">Confirmado por Yext: tu ficha ya alimenta a otras plataformas</div>
    ${filas}
  </div>`;
}
...
function directoriosGrillaHtml(yx) {
  ...
  return ordenados.map((g) => {
    ...
    const beneficio = BENEFICIOS_POR_TIPO[g.tipo] || '';
    return `<div class="dir-grupo">
      ...
      ${beneficio ? `<p class="dir-grupo-beneficio">${esc(beneficio)}</p>` : ''}
...
function directoriosSectionHtml(d) {
  ...
  <div class="section page-start">
    ...
    <p class="dir-intro">Revisamos ${yx.totalAnalizados} directorios, mapas y plataformas donde tus clientes buscan negocios como el tuyo — no todos pesan igual para tu rubro, pero cuantos más tengan tu nombre, dirección y teléfono exactos, más confían Google y las IA en que sos un negocio real, y antes te muestran.</p>
    ...
    ${integracionesDestacadasHtml(yx)}
...
<div class="page">
  ...
  <div class="section modules-section page-start">
    ...
    <div class="modules-grid">${modulos.map((m, i) => {
      if (i === 2) return moduleCardHtml(m) + saludContenidoHtml(d) + buenasPracticasAisoHtml();
      if (i === 3) return moduleCardHtml(m) + publicidadRealHtml(d);
      return moduleCardHtml(m);
    }).join('')}</div>
  </div>
...
.dir-grupo-beneficio { font-size: 11px; color: var(--guru-text-secondary); line-height: 1.4; margin: 4px 0 10px; font-style: italic; }
.integ-box { background: linear-gradient(135deg, #EEF2FF, #F5F0FF); border: 1px solid var(--guru-purple); border-radius: var(--radius-md); padding: 14px 18px; margin-bottom: 18px; }
.integ-title { font: 700 12px var(--font-display); color: var(--guru-purple); margin-bottom: 8px; }
.integ-fila { font-size: 12px; color: var(--guru-text-primary); line-height: 1.7; }
.integ-fila strong { color: var(--guru-purple); }

/* Salud del contenido */
.cq-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-bottom: 14px; }
.cq-card { background: var(--guru-bg); border-radius: var(--radius-md); padding: 14px 16px; }
.cq-card-label { font-size: 10.5px; color: var(--guru-text-secondary); font-weight: 600; text-transform: uppercase; letter-spacing: .03em; margin-bottom: 6px; }
.cq-card-value { font: 800 22px var(--font-display); }
.cq-card-detail { font-size: 11px; color: var(--guru-text-secondary); margin-top: 4px; line-height: 1.4; }
.cq-list { background: var(--guru-surface); border: 1px solid var(--guru-border); border-radius: var(--radius-md); padding: 4px 16px; margin-bottom: 14px; }
.cq-row { display: flex; align-items: flex-start; gap: 10px; padding: 10px 0; border-bottom: 1px solid var(--guru-border); font-size: 12px; line-height: 1.5; }
.cq-row:last-child { border-bottom: none; }
.cq-icon { flex-shrink: 0; margin-top: 1px; }
.cq-row-text { color: var(--guru-text-secondary); }
.cq-row-text strong { color: var(--guru-text-primary); }
.cq-cross { background: linear-gradient(135deg, #EEF2FF, #F5F0FF); border: 1px solid var(--guru-purple); border-radius: var(--radius-md); padding: 12px 16px; font-size: 11.5px; color: var(--guru-text-primary); line-height: 1.6; }
.cq-cross strong { color: var(--guru-purple); }

/* Buenas prácticas AISO */
.bp-list { margin-bottom: 14px; }
.bp-item { display: flex; gap: 12px; padding: 10px 0; border-bottom: 1px dashed var(--guru-border); }
.bp-item:last-child { border-bottom: none; }
.bp-num { flex-shrink: 0; width: 24px; height: 24px; border-radius: 50%; background: var(--guru-purple); color: #fff; font: 700 12px var(--font-display); display: flex; align-items: center; justify-content: center; }
.bp-item strong { font-size: 12.5px; color: var(--guru-text-primary); display: block; margin-bottom: 2px; }
.bp-item p { font-size: 11.5px; color: var(--guru-text-secondary); margin: 0; line-height: 1.5; }

/* Publicidad Real */
.ad-fecha-verificacion { margin-left: auto; font-size: 10.5px; color: var(--guru-teal); font-weight: 600; }
.ad-plataforma-card { background: var(--guru-bg); border-radius: var(--radius-md); padding: 14px 16px; margin-bottom: 12px; page-break-inside: avoid; break-inside: avoid; }
.ad-plataforma-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; }
.ad-plataforma-nombre { font: 700 13px var(--font-display); color: var(--guru-text-primary); }
.ad-plataforma-conteo { font-weight: 400; color: var(--guru-text-secondary); font-size: 11.5px; }
.ad-plataforma-badge { font: 700 10px var(--font-display); letter-spacing: .04em; padding: 3px 10px; border-radius: 999px; white-space: nowrap; }
.ad-plataforma-badge.si { background: rgba(15,155,142,.14); color: #0B7F74; }
.ad-plataforma-badge.no { background: rgba(148,163,184,.18); color: var(--guru-text-secondary); }
.ad-plataforma-vacio { font-size: 11.5px; color: var(--guru-text-muted); font-style: italic; margin: 4px 0; }
.ad-plataforma-mas { font-size: 10.5px; color: var(--guru-text-muted); margin: 8px 0 0; font-style: italic; }
.ad-plataforma-fuente { font-size: 9.5px; color: var(--guru-text-muted); margin-top: 10px; }
.ad-creativos-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }
.ad-creativo-card { background: var(--guru-surface); border: 1px solid var(--guru-border); border-radius: var(--radius-sm); overflow: hidden; display: flex; flex-direction: column; page-break-inside: avoid; break-inside: avoid; }
.ad-creativo-media { width: 100%; height: 130px; flex-shrink: 0; background: #EEF0F5; display: flex; align-items: center; justify-content: center; overflow: hidden; }
.ad-creativo-media.screenshot { height: 150px; } /* las capturas de Google (anuncio + texto) necesitan más alto que una foto de Meta */
.ad-creativo-img { width: 100%; height: 100%; object-fit: contain; background: #fff; display: block; } /* contain siempre: se ve la creatividad completa, nunca recortada */
.ad-creativo-placeholder { color: var(--guru-text-muted); font-size: 11px; }
.ad-creativo-body { padding: 8px 9px 9px; flex: 1; display: flex; flex-direction: column; gap: 3px; }
.ad-creativo-formato { align-self: flex-start; font: 700 8.5px var(--font-display); letter-spacing: .03em; text-transform: uppercase; color: var(--guru-purple); background: rgba(108,99,255,.1); padding: 1px 6px; border-radius: 999px; }
.ad-creativo-texto { font-size: 10px; color: var(--guru-text-secondary); line-height: 1.4; margin: 0; }
.ad-creativo-fechas { font-size: 9px; color: var(--guru-text-muted); margin-top: auto; padding-top: 4px; }
.ad-recos-box { background: linear-gradient(135deg, #EEF2FF, #F5F0FF); border: 1px solid var(--guru-purple); border-radius: var(--radius-md); padding: 14px 18px; margin-top: 4px; }
.ad-recos-title { font: 700 13px var(--font-display); color: var(--guru-purple); margin-bottom: 8px; }
...
.ia-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-bottom: 22px; }
...
.ia-card-quote { font-size: 11.5px; line-height: 1.5; font-style: italic; color: var(--guru-text-primary); background: var(--guru-bg); border-radius: var(--radius-md); padding: 9px 11px; margin-bottom: 10px; }
.ia-card-fuente { display: inline-flex; align-items: center; gap: 4px; font-size: 10px; color: var(--guru-purple); text-decoration: none; margin-bottom: 10px; }
.ia-verif { display: inline-flex; align-items: center; gap: 4px; font: 700 9.5px var(--font-display); letter-spacing: .04em; padding: 3px 8px; border-radius: 999px; margin-top: 10px; }
.ia-verif-real { background: rgba(15,155,142,.13); color: #0B7F74; }
.ia-verif-est { background: rgba(245,166,35,.14); color: #B4780C; }
...
.chat-sim-tag { font: 700 9.5px var(--font-display); letter-spacing: .1em; text-transform: uppercase; background: var(--guru-gold); color: white; padding: 4px 9px; border-radius: 4px; display: inline-flex; align-items: center; gap: 4px; }
.chat-sim-tag-real { background: #0F9B8E; }
.chat-sim-fecha { margin-left: auto; font-size: 10.5px; color: var(--guru-text-muted); font-weight: 600; }
.aiso-urgencia { display: flex; align-items: center; gap: 10px; background: linear-gradient(135deg, #E94560, #C2334D); color: #fff; border-radius: var(--radius-md); padding: 14px 18px; margin-bottom: 20px; font-size: 13.5px; line-height: 1.5; }
...
.chat-cols { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 4px 0 8px; }
.chat-col { background: linear-gradient(135deg, #F5F3FF, #FFF1F4); border: 1px solid rgba(108,99,255,.18); border-radius: 12px; padding: 12px 13px; display: flex; flex-direction: column; gap: 8px; page-break-inside: avoid; break-inside: avoid; }
.chat-col-head { display: flex; align-items: center; gap: 7px; }
.chat-col-head .chat-avatar-ai { width: 22px; height: 22px; }
.chat-col-head .chat-avatar-ai svg { width: 12px; height: 12px; }
.chat-col-label { font: 700 11.5px var(--font-display); color: var(--guru-text-primary); }
.chat-col-text { font-size: 10px; line-height: 1.55; color: var(--guru-text-secondary); margin: 0; }
.chat-col-top3 { margin-top: auto; padding-top: 8px; border-top: 1px solid rgba(108,99,255,.15); }
.chat-col-top3-label { font: 700 8.5px var(--font-display); letter-spacing: .04em; text-transform: uppercase; color: var(--guru-purple); }
.chat-col-top3 ol { margin: 4px 0 0; padding-left: 15px; }
.chat-col-top3 li { font-size: 9.5px; line-height: 1.5; color: var(--guru-text-secondary); }
