...
function creativoCardHtml(item) {
  ...
  const mediaHtml = imagen
    ? `<img class="ad-creativo-img${esScreenshot ? ' contain' : ''}" src="${esc(imagen)}" />`
    : `<div class="ad-creativo-placeholder">${formatoEtiqueta === 'Video' ? '🎬 Video' : '🖼️ Imagen'}</div>`;

  return `<div class="ad-creativo-card">
    <div class="ad-creativo-media">${mediaHtml}</div>
...
.ad-creativo-media { width: 100%; height: 100px; flex-shrink: 0; background: #EEF0F5; display: flex; align-items: center; justify-content: center; }
