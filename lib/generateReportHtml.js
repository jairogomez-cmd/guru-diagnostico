...
function competidoresSectionHtml(d) {
  ...
  const rows = todos.map((t) => {
    ...
    return `<div class="comp-row${t.isClient ? ' comp-row-client' : ''}">
      ...
      <div class="comp-meta">${t.rating != null ? `★ ${t.rating}` : '—'} · ${(t.resenas || 0).toLocaleString('es-AR')} res.${t.tieneWebsite === false ? ' · sin web' : ''}</div>
    </div>`;
  }).join('');
...
function oportunidadPerdidaHtml(d) {
  ...
  const ejemplos = [50000, 300000, 1500000];
  const filasEjemplo = ejemplos.map((ticket) => {
    const ingreso = Math.round(ventasPerdidasPor100 * ticket);
    return `<div class="ejemplo-row">
      <div class="ejemplo-ticket">Ticket promedio<br><strong>$${ticket.toLocaleString('es-AR')}</strong></div>
      <div class="ejemplo-arrow">→</div>
      <div class="ejemplo-resultado">~$${ingreso.toLocaleString('es-AR')}<span>/mes no capturados (ejemplo)</span></div>
    </div>`;
  }).join('');
  ...
  <div class="section page-start">
    ...
    <div class="op-ejemplo-box">
      <div class="op-ejemplo-label">Ejemplo ilustrativo — hacé la cuenta con tus propios números</div>
      <p class="op-ejemplo-intro">Si tu categoría suele cerrar en ventas alrededor del ${tasaCierre}% de los contactos que recibe, esas ${pct} oportunidades perdidas (de cada 100) podrían representar aproximadamente <strong>${ventasPerdidasPor100} ventas menos al mes</strong>:</p>
...
.cliente-logo-card { display: inline-flex; align-items: center; justify-content: center; margin-top: 16px; background: white; border-radius: 14px; padding: 12px 24px; box-shadow: 0 8px 24px rgba(0,0,0,.3); min-height: 64px; }
.cliente-logo-img { display: block; max-height: 64px; max-width: 280px; width: auto; height: auto; object-fit: contain; }
...
.module-card, .radar-item, .chart-box, .opp-card, .aiso-box, .questions-box, .detail-card, .hero, .comp-box, .op-headline-box, .op-ejemplo-box, .op-fuentes-box {
