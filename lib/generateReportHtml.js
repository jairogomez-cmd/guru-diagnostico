...
const MODULOS_DEF = [
  { key: 'seoLocal', label: 'SEO Local', peso: 0.20, color: 'var(--guru-teal)' },
  { key: 'posicionamiento', label: 'Posicionamiento', peso: 0.15, color: '#4FBEB0' },
  { key: 'construccion', label: 'Construcción', peso: 0.20, color: 'var(--guru-gold)' },
  { key: 'publicidad', label: 'Publicidad', peso: 0.10, color: 'var(--guru-accent)' },
  { key: 'aiso', label: 'AISO', peso: 0.15, color: 'var(--guru-purple)' },
  { key: 'conversion', label: 'Conversión', peso: 0.10, color: '#9B8CFF' },
  { key: 'redes', label: 'Redes', peso: 0.10, color: '#FF9B8C' },
];
...
function radarChartSvg(scores) {
  ...
  const axes = MODULOS_DEF.map((it, i) => {
    ...
    return `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="#E7E6F5" stroke-width="1"/>
      <text x="${lx}" y="${ly}" text-anchor="middle" font-size="9.5" fill="#64748B" font-family="DM Sans,sans-serif">${esc(it.label)}</text>`;
  }).join('');
  ...
  return `<svg viewBox="0 0 300 320" width="100%" style="max-width:300px;display:block">
    ${rings}${axes}
...
function moduloSeoLocal(d) {
  ...
  return { titulo: 'SEO Local', score: d.scores?.seoLocal, chips: chipsHtml(directorios), extra: ratingBlock(gbp), items, next, porque };
}
...
function moduloPublicidad(d) {
  ...
  return { titulo: 'Publicidad', score: d.scores?.publicidad, items, next, porque };
}
...
function logoHeaderHtml(imagenes) {
  const logo = imagenes?.logo_cliente || null;
  if (!logo) return '';
  return `<div class="cliente-logo-wrap"><img src="${esc(logo)}" class="cliente-logo" alt="Logo del negocio" /></div>`;
}
function fotoHtml(imagenes) {
  const foto = imagenes?.foto_gbp || imagenes?.og_image || null;
  if (foto) return `<img src="${esc(foto)}" class="hero-photo-img" alt="Foto del negocio" />`;
  return `<div class="hero-photo">Sin foto disponible</div>`;
}

function websiteMockupHtml(website, screenshotDataUri) {
  if (!screenshotDataUri) return '';
  return `
  <div class="section">
    <div class="section-eyebrow">Así se ve hoy</div>
    <div class="section-title">Tu sitio web en vivo</div>
    <div class="browser-mockup">
      <div class="browser-bar">
        ...
        <span class="browser-url">${esc((website || '').replace(/^https?:\/\//, ''))}</span>
      </div>
      <div class="browser-screenshot-frame">
        <img src="${screenshotDataUri}" alt="Captura del sitio web" />
        <div class="browser-screenshot-fade"></div>
      </div>
    </div>
  </div>`;
}
...
// FUNCIÓN PRINCIPAL
// ══════════════════════════════════════════════════════════════
function generateReportHtml(d, extra) {
  extra = extra || {};
...
<div class="page">
  ...
  ${websiteMockupHtml(d.website, extra.screenshotDataUri)}
...
.hero-photo, .hero-photo-img { width: 88px; height: 88px; border-radius: var(--radius-md); flex-shrink: 0; object-fit: cover; }
...
.cliente-logo-wrap { margin-top: 14px; }
.cliente-logo { width: 76px; height: 76px; border-radius: 12px; object-fit: contain; background: white; padding: 8px; box-shadow: 0 6px 18px rgba(0,0,0,.25); }
...
.browser-mockup { border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow-card); border: 1px solid var(--guru-border); page-break-inside: avoid; break-inside: avoid; }
.browser-bar { display: flex; align-items: center; gap: 6px; padding: 10px 14px; background: #EDEFF4; }
.browser-dot { width: 10px; height: 10px; border-radius: 50%; }
.browser-url { margin-left: 10px; background: white; border-radius: 999px; padding: 3px 14px; font-size: 11px; color: var(--guru-text-secondary); flex: 1; max-width: 320px; }
.browser-screenshot-frame { position: relative; width: 100%; height: 420px; overflow: hidden; background: var(--guru-bg); }
.browser-screenshot-frame img { position: absolute; top: 0; left: 0; width: 100%; height: auto; }
.browser-screenshot-fade { position: absolute; bottom: 0; left: 0; width: 100%; height: 90px; background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,.97)); }
