...
function coreWebVitalsHtml(pagespeed) {
  ...
  const items = [
    { label: 'Carga visual (LCP)', val: `${(lcp / 1000).toFixed(1)}s`, st: cwvStatus('lcp', lcp) },
    { label: 'Estabilidad visual (CLS)', val: cls != null ? cls.toFixed(2) : '—', st: cls != null ? cwvStatus('cls', cls) : null },
    { label: 'Interactividad (TBT)', val: tbt != null ? `${Math.round(tbt)}ms` : '—', st: tbt != null ? cwvStatus('tbt', tbt) : null },
  ];
  return `<div class="detail-card detail-card-wide">
    ...
    <div class="mini-scores">${items.map((it) => `
      <div class="mini-score">
        <div class="mini-score-val" style="color:${it.st ? it.st.hex : '#94A3B8'}">${it.val}</div>
        <div class="mini-score-lbl">${esc(it.label)}</div>
        ${it.st ? `<div style="font-size:10px;font-weight:700;color:${it.st.hex};margin-top:2px;">${it.st.label}</div>` : ''}
      </div>`).join('')}</div>
    <div class="detail-card-note" style="margin-top:8px;">Estas son las 3 métricas que Google usa para decidir tu posición en resultados de búsqueda en celular — no son opiniones, son datos que Google mide en tiempo real sobre tu sitio.</div>
  </div>`;
}
...
function moduloSeoLocal(d) {
  ...
  return { titulo: 'SEO Local', score: d.scores?.seoLocal, chips: chipsHtml(directorios), extra: ratingBlock(gbp), tarjetas: tarjetas.join(''), items, next, porque };
}
...
function logoBadgeHtml(imagenes) {
  const logo = imagenes?.logo_cliente || null;
  if (!logo) return '';
  return `<img src="${esc(logo)}" class="cliente-logo-badge" alt="Logo del negocio" onerror="this.style.display='none'" />`;
}
...
.hero-photo-wrap { position: relative; width: 124px; height: 124px; flex-shrink: 0; }
.hero-photo, .hero-photo-img { width: 124px; height: 124px; border-radius: 20px; object-fit: cover; box-shadow: 0 10px 26px rgba(0,0,0,.35); }
...
.cliente-logo-badge { position: absolute; bottom: -12px; right: -12px; width: 54px; height: 54px; border-radius: 14px; background: white; padding: 7px; box-shadow: 0 6px 18px rgba(0,0,0,.32); object-fit: contain; border: 2.5px solid rgba(255,255,255,.95); }
