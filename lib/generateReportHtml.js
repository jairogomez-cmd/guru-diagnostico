...
function logoHeaderHtml(imagenes) {
  ...
  if (!logo) return '';
  return `<div class="cliente-logo-wrap"><img src="${esc(logo)}" class="cliente-logo" alt="Logo del negocio" onerror="this.parentElement.style.display='none'" /></div>`;
}
...
<div class="page">
  ...
  <div class="hero">
    ...
    <div class="guru-logo-corner"><img src="https://guru-diagnostico.vercel.app/logo-guru.png" class="guru-logo-img" alt="Guru Soluciones" onerror="this.style.display='none';this.nextElementSibling.style.display='block'" /><div class="guru-logo" style="display:none">GURU<span>·</span>SOLUCIONES</div></div>
  </div>
...
.guru-logo-wrap { display: flex; align-items: center; }
.guru-logo-corner { position: absolute; bottom: 20px; right: 48px; z-index: 2; background: rgba(255,255,255,.1); backdrop-filter: blur(4px); padding: 8px 14px; border-radius: var(--radius-md); }
.guru-logo-img { height: 26px; width: auto; display: block; }
.guru-logo { font: 800 17px var(--font-display); letter-spacing: -0.02em; color: white; }
...
.cliente-logo { width: 108px; height: 108px; border-radius: 14px; object-fit: contain; background: white; padding: 10px; box-shadow: 0 8px 22px rgba(0,0,0,.28); }
