...
function logoHeaderHtml(imagenes) {
  ...
  if (!logo) return '';
  return `<img src="${esc(logo)}" class="cliente-logo" alt="Logo del negocio" />`;
}
...
function websiteMockupHtml(website, screenshotDataUri) {
  ...
  <div class="section">
    ...
    <div class="browser-mockup">
      ...
      <img src="${screenshotDataUri}" class="browser-screenshot" alt="Captura del sitio web" />
    </div>
  </div>`;
}
...
.cliente-logo { position: absolute; top: -6px; right: 0; width: 40px; height: 40px; border-radius: 8px; object-fit: contain; background: white; padding: 4px; box-shadow: 0 4px 12px rgba(0,0,0,.25); }
...
.browser-mockup { border-radius: var(--radius-lg); overflow: hidden; box-shadow: var(--shadow-card); border: 1px solid var(--guru-border); }
